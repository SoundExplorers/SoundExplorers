...
namespace SoundExplorersDatabase.Tests.Data {
  ...
  public class KeyTests {
    ...
    public void T020_Difference() {
      var location1 = new MockLocation {Name = "Fred's"};
      var location2 = new MockLocation {Name = "Pyramid Club"};
      ...
      var event1 = new MockEvent {Date = DateTime.Today, Location = location1};
      var event2 = new MockEvent {Date = event1.Date, Location = location2};
      Assert.IsTrue(event1.Key != event2.Key, "event1.Key != event2.Key");
      Assert.IsTrue(event1.Key < event2.Key, "event1.Key < event2.Key");
      Assert.IsTrue(event2.Key > event1.Key, "event2.Key > event1.Key");
      var event3 = new MockEvent {Date = DateTime.Today, Location = location1};
      var event4 = new MockEvent
      ...
      var event5 = new MockEvent {Date = DateTime.Today};
      var event6 = new MockEvent {Date = event1.Date, Location = location2};
      Assert.IsTrue(event5.Key < event6.Key, "event5.Key < event6.Key");
      var set1 = new MockSet {SetNo = 1, Event = event1};
      var set2 = new MockSet {SetNo = 1, Event = event2};
    ...
    public void T030_ToString() {
      ...
      event1.Location = new MockLocation {Name = locationName};
      Assert.AreEqual(event1.Key.ToString(), "1800/12/25 Fred's",
        "SimpleKey and Identifying Parent");
      var set1 = new MockSet {SetNo = setNo, Event = event1};
