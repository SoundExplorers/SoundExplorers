...
namespace SoundExplorersDatabase.Tests.Data {
  ...
  public class ReferencingTests {
    ...
    public void Setup() {
      ...
      Parent1 = new Parent() {Name = Parent1Name};
      Parent2 = new Parent() {Name = Parent2Name};
      Child1 = new Child() {Name = Child1Name};
      Child2 = new Child() {Name = Child2Name};
    ...
    [Test]
    public void T10_Parent1Child1() {
      AddChild1ToParent1();
      Assert.AreSame(Parent1, Child1.Parent, "Child1.Parent");
      Assert.IsTrue(Child1.IsPersistent, "Child1.IsPersistent");
      Assert.AreEqual(Child1Name, Child1.Name, "Child1.Name");
      Assert.IsTrue(Parent1.IsPersistent, "Parent1.IsPersistent");
      Assert.AreEqual(Parent1Name, Parent1.Name, "Parent1.Name");
      Assert.AreEqual(1, Parent1.Children.Count, "Parent1.Children.Count");
      Assert.AreEqual(1, Parent1.References.Count,
        "Parent1.References.Count after Add #1");
    }

    private void AddChild1ToParent1() {
      using (var session = new SessionNoServer(DatabaseFolderPath)) {
        session.BeginUpdate();
        Parent1 = Parent.Read(Parent1Name, session);
        Child1 = Child.Read(Child1Name, session);
        //Parent1.Children.Add(Child1);
        Assert.AreSame(Child1, Parent1.Children.First(), "Parent 1 Child1");
        session.Commit();
      }
    }
