  ...
  internal class SchemaVersionTests {
    ...
    public void Upgrade() {
      ...
      using (var session = new TestSession
        ...
        var version = SchemaVersion.Read(1, session);
        Assert.AreEqual(0, version.Number, "Number before upgrade 1.");
        Assert.IsFalse(version.IsUpToDate,
          "SchemaVersion up to date before upgrade 1.");
        version.Upgrade();
        Assert.AreEqual(1, version.Number, "Number after upgrade 1.");
        Assert.IsTrue(version.IsUpToDate,
          "SchemaVersion up to date after upgrade 1.");
        version = SchemaVersion.Read(2, session);
        version.Upgrade();
        Assert.AreEqual(2, version.Number, "Number after upgrade 2.");
        Assert.IsTrue(version.IsUpToDate,
          "SchemaVersion up to date after upgrade 2.");
        Assert.IsTrue(
          Directory.Exists(databaseFolderPath),
