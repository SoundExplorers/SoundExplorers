...
CREATE  TABLE IF NOT EXISTS RoleInPiece (
  PerformanceDate DATE NOT NULL ,
  LocationId VARCHAR(3) NOT NULL ,
  SetNo INT NOT NULL ,
  PieceNo INT NOT NULL ,
  ArtistName VARCHAR(255) NOT NULL ,
  RoleId VARCHAR(3) NOT NULL ,
  CONSTRAINT PK_RoleInPiece PRIMARY KEY (PerformanceDate, LocationId, SetNo, PieceNo, ArtistName, RoleId) ,
  CONSTRAINT FK_RoleInPiece_Artist
    FOREIGN KEY (ArtistName )
    REFERENCES Artist (ArtistName )
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
  CONSTRAINT FK_RoleInPiece_Role
    FOREIGN KEY (RoleId )
    REFERENCES Role (RoleId )
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
  CONSTRAINT FK_RoleInPiece_Piece
