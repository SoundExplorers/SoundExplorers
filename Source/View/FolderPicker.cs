...
#pragma warning restore IDE1006 // Naming Styles
    [PublicAPI]
    public virtual string ResultPath { get; protected set; } = null!;
    
    [PublicAPI]
    public virtual string ResultName { get; protected set; } = null!;
    public virtual string InputPath { get; set; } = null!;
    ...
    public virtual bool? ShowDialog(IntPtr owner, bool throwOnError = false) {
      ...
      if (!string.IsNullOrEmpty(InputPath)) {
        if (CheckHr(
          SHCreateItemFromParsingName(InputPath, null, typeof(IShellItem).GUID,
            out var item), throwOnError) != 0) {
          return null;
        }
      ...
      if (CheckHr(result.GetDisplayName(SIGDN.SIGDN_DESKTOPABSOLUTEPARSING, out var path),
        throwOnError) != 0) {
        return null;
      }
      ResultPath = path;
      if (CheckHr(result.GetDisplayName(SIGDN.SIGDN_DESKTOPABSOLUTEEDITING, out path),
        false) == 0) {
        ResultName = path;
      }
      return true;
    ...
    private interface IShellItem {
      ...
      [PreserveSig]
      int GetDisplayName(SIGDN sigdnName,
