...
namespace SoundExplorers {
    ...
    internal partial class TableForm : Form {
        ...
        public TableForm(
            ...
            GridSplitContainer.GotFocus += new EventHandler(SplitContainer_GotFocus);
            ImageSplitContainer.GotFocus += new EventHandler(SplitContainer_GotFocus);
            TableName = tableName;
            OpenTable(TableName);
        }
        ...
        private void Entities_RowUpdated(object sender, RowUpdatedEventArgs e) {
            ...
            ToolStripStatusLabel statusLabel = (ParentForm as MdiParentForm).StatusLabel;
        ...
        private void FittedPictureBox1_DragDrop(object sender, DragEventArgs e) {
            if (Entities is SoundExplorers.Data.ImageList
            &&  !MainGrid.IsCurrentCellInEditMode
        ...
        private void FittedPictureBox1_MouseDown(object sender, MouseEventArgs e) {
            var data = new DataObject(
                ...
                new string[] { FittedPictureBox1.ImageLocation });
        ...
        private void Grid_MouseDown(object sender, MouseEventArgs e) {
            ...
            if (e.Button == MouseButtons.Right) {
                ...
                DataGridViewCell cell = GetCellAtClientCoOrdinates(e.X, e.Y);
                if (cell != null) { // Cell found
                    ...
                    if (pathCell != null && pathCell.FileExists) { 
                        ...
                        var data = new DataObject(DataFormats.FileDrop, new string[] { pathCell.Path });
        ...
        private void MainGrid_CellEndEdit(object sender, DataGridViewCellEventArgs e) {
            ...
            DataGridViewColumn column = pathCell.OwningColumn;
            //Debug.WriteLine("MainGrid_CellEndEdit " + column.Name);
            if (Entities is SoundExplorers.Data.ImageList) { // Image.Path
            ...
            && (    UnchangedRow == null 
                ...
                if (pathCell.FileExists) {
                    ...
                    case "Path":
                        if (Entities is SoundExplorers.Data.ImageList) { // Image.Path
                            SoundExplorers.Data.Image.DefaultFolder = file.Directory;
        ...
        private void MainGrid_DragDrop(object sender, DragEventArgs e) {
            ...
            Point clientCoOrdinates = MainGrid.PointToClient(new Point(e.X, e.Y));
            var pathCell =
        ...
        private void MainGrid_RowEnter(object sender, DataGridViewCellEventArgs e) {
            ...
            if (Entities is CreditList) {
                ...
                if (parentPiece != null) {
                    ...
                    parentPiece.Credits = new CreditList(empty: true);
                    foreach (Credit credit in credits) {
                        parentPiece.Credits.Add(credit);
                    }
                    parentPiece.Original = parentPiece.Clone();
                }
            } else if (Entities is SoundExplorers.Data.ImageList) {
                var image = Entities[e.RowIndex] as SoundExplorers.Data.Image;
        ...
        private void MainGrid_RowValidated(object sender, DataGridViewCellEventArgs e) {
            ...
            try {
                foreach (DataGridViewColumn column in MainGrid.Columns) {
                    ...
                    object newValue = MainGrid.Rows[e.RowIndex].Cells[column.Name].Value;
                    string newString = newValue != null ? newValue.ToString() : string.Empty;
                    object oldValue = UnchangedRow != null ? UnchangedRow.Cells[column.Index].Value : null;
                    ...
                    if (newValue == DBNull.Value) {
                        if (column.ValueType == typeof(DateTime)) {
                            ...
                            MainGrid.Rows[e.RowIndex].Cells[column.Name].Value =
                                SoundExplorers.Data.Image.GetNextImageId();
                        }
                    }
                } // End of foreach
                if (isUpdateRequired) {
        ...
        private void ParentGrid_RowEnter(object sender, DataGridViewCellEventArgs e) {
            ...
            if (Entities.ParentList is SoundExplorers.Data.ImageList
            &&  e.RowIndex < Entities.ParentList.Count) {
                var image = Entities.ParentList[e.RowIndex] as SoundExplorers.Data.Image;
                ShowImageOrMessage(image.Path);
            }
        }
        ...
        private void RowErrorTimer_Tick(object sender, EventArgs e) {
            ...
            try {
                ...
                try {
                    ...
                    Debug.WriteLine("RowErrorEventArgs.ColumnIndex = " + RowErrorEventArgs.ColumnIndex);
                    Debug.WriteLine("RowErrorEventArgs.RowIndex = " + RowErrorEventArgs.RowIndex);
                    Debug.WriteLine("MainGrid.CurrentCell.ColumnIndex = " + MainGrid.CurrentCell.ColumnIndex);
                    Debug.WriteLine("MainGrid.CurrentRow.Index = " + MainGrid.CurrentRow.Index);
            ...
            for (int columnIndex = 0; columnIndex < Entities.Columns.Count; columnIndex++) {
                object rejectedValue = RowErrorEventArgs.RejectedValues[columnIndex];
        ...
        private void TableForm_FormClosed(object sender, FormClosedEventArgs e) {
            ...
            || Entities is SoundExplorers.Data.ImageList) {
        ...
        private DataGridViewCell GetCellAtClientCoOrdinates(int x, int y) {
            DataGridView.HitTestInfo hitTestInfo = MainGrid.HitTest(x, y);
        ...
        private Newsletter GetNewsletterToShow() {
            if (Entities is ArtistInImageList) {
                if (Entities.ParentList.Count > 0) {
                    var image =
                        (SoundExplorers.Data.Image)Entities.ParentList[ParentGrid.CurrentRow.Index];
            ...
            } else if (Entities is CreditList) {
                if (Entities.ParentList.Count > 0) {
                    var piece =
                ...
                var image = 
                    (SoundExplorers.Data.Image)Entities[MainGrid.CurrentRow.Index];
                ...
                return image.FetchNewsletter();
            } else if (Entities is NewsletterList) {
                if (MainGrid.CurrentRow.IsNewRow) {
                    throw new ApplicationException(
            ...
            } else if (Entities is PerformanceList) {
                if (MainGrid.CurrentRow.IsNewRow) {
                    throw new ApplicationException(
                ...
                Performance performance = (Performance)Entities[MainGrid.CurrentRow.Index];
                if (performance.Newsletter != (DateTime)MainGrid.CurrentRow.Cells["Newsletter"].Value) {
                    throw new ApplicationException(
                        "You must save or cancel changes to the Performance " 
                        + "before you can show its newsletter.");
                }
                return performance.FetchNewsletter();
            } else if (Entities is SetList) {
                if (Entities.ParentList.Count > 0) {
                    var performance = 
        ...
        private void InvertGridColors(DataGridView grid) {
            Color swapColor = grid.DefaultCellStyle.BackColor;
        ...
        private void PopulateGrid(string tableName) {
            ...
            Entities.RowError += new EventHandler<RowErrorEventArgs>(Entities_RowError);
            Entities.RowUpdated += new EventHandler<RowUpdatedEventArgs>(Entities_RowUpdated);
            this.Text = Entities.TableName;

            MainGrid.CellBeginEdit -= new DataGridViewCellCancelEventHandler(MainGrid_CellBeginEdit);
            MainGrid.CellEndEdit -= new DataGridViewCellEventHandler(MainGrid_CellEndEdit);
            ...
            MainGrid.Click -= new EventHandler(Grid_Click);
            MainGrid.DataError -= new DataGridViewDataErrorEventHandler(MainGrid_DataError);
            MainGrid.DragDrop -= new DragEventHandler(MainGrid_DragDrop);
            ...
            MainGrid.MouseDown -= new MouseEventHandler(Grid_MouseDown);
            //MainGrid.RowStateChanged
            MainGrid.RowEnter -= new DataGridViewCellEventHandler(MainGrid_RowEnter);
            MainGrid.RowsRemoved -= new DataGridViewRowsRemovedEventHandler(MainGrid_RowsRemoved);
            MainGrid.RowValidated -= new DataGridViewCellEventHandler(MainGrid_RowValidated);
            ...
            if (Entities.ParentList != null) {
                ...
                MainGrid.DataSource = new BindingSource(
                        dataSource: ParentGrid.DataSource,
                        dataMember: Entities.DataSet.Relations[0].RelationName);
                foreach (IEntityColumn entityColumn in Entities.Columns) {
                    if (entityColumn.ColumnName != "Comments") { // Comments is on multiple tables
                        ...
                        || Entities.ParentList.Columns.ContainsKey(entityColumn.ReferencedColumnName)) {
            ...
            foreach (DataGridViewColumn column in MainGrid.Columns) {
                IEntityColumn entityColumn = Entities.Columns[column.Index];
        ...
        private void PopulateParentGrid() {
            ParentGrid.Click -= new EventHandler(Grid_Click);
            ParentGrid.CurrentCellChanged -= new EventHandler(ParentGrid_CurrentCellChanged);
            //ParentGrid.GotFocus -= new EventHandler(Control_GotFocus);
            //ParentGrid.LostFocus -= new EventHandler(Control_LostFocus);
            ParentGrid.MouseDown -= new MouseEventHandler(Grid_MouseDown);
            ParentGrid.RowEnter -= new DataGridViewCellEventHandler(ParentGrid_RowEnter);
            ParentGrid.DataSource = new BindingSource(
                    dataSource: Entities.DataSet,
                    dataMember: Entities.ParentList.TableName);
            foreach (DataGridViewColumn column in ParentGrid.Columns) {
                IEntityColumn entityColumn = Entities.ParentList.Columns[column.Index];
                column.Visible = entityColumn.Visible;
                if (column.Visible) {
                    if (column.ValueType == typeof(DateTime)) {
                        column.DefaultCellStyle.Format = "dd MMM yyyy";
            ...
            //ParentGrid.GotFocus += new EventHandler(Control_GotFocus);
            //ParentGrid.LostFocus += new EventHandler(Control_LostFocus);
            ParentGrid.MouseDown += new System.Windows.Forms.MouseEventHandler(Grid_MouseDown);
            ParentGrid.RowEnter += new DataGridViewCellEventHandler(ParentGrid_RowEnter);
            if (ParentGrid.RowCount > 0) {
                ParentGrid.CurrentCell = ParentGrid.Rows[0].Cells[0]; // Triggers ParentGrid_RowEnter
            }
        }
        ...
        private void SwapGridColors() {
            Color swapColor = MainGrid.DefaultCellStyle.BackColor;
            ...
            MainGrid.DefaultCellStyle.SelectionBackColor = ParentGrid.DefaultCellStyle.SelectionBackColor;
            ParentGrid.DefaultCellStyle.SelectionBackColor = swapColor;
            swapColor = MainGrid.DefaultCellStyle.SelectionForeColor;
            MainGrid.DefaultCellStyle.SelectionForeColor = ParentGrid.DefaultCellStyle.SelectionForeColor;
