...
namespace SoundExplorers.Data {
    ...
    internal class Entity<T> : IEntity
        ...
        public virtual T Clone() {
            T that = Factory<T>.Create(typeof(T));
        ...
        public virtual void Delete() {
            foreach (EntityColumn<T> primaryKeyColumn in PrimaryKeyColumns) {
                Adapter.DeleteCommand.Parameters["@" + primaryKeyColumn.ColumnName].Value =
            ...
            try {
                ...
                throw new DataException(
                    "Error on executing SQL command to delete " + TableName + ":" + Environment.NewLine
        ...
        public virtual bool Fetch() {
            foreach (EntityColumn<T> primaryKeyColumn in PrimaryKeyColumns) {
                Adapter.SelectCommand.Parameters["@" + primaryKeyColumn.ColumnName].Value =
                    primaryKeyColumn.GetValue(this);
            } // End of foreach
            ...
            try {
                ...
                throw new DataException(
                    "Error on executing SQL command to fetch " + TableName + ":" + Environment.NewLine
                    + ex.Message + Environment.NewLine + Environment.NewLine
                    + "SQL command text:" + Environment.NewLine + Environment.NewLine
        ...
        public virtual void Save() {
            ...
            if (hasbeenFetched) {
                foreach (EntityColumn<T> column in Columns) {
                    ...
                    if (column.IsInPrimaryKey) {
                        Adapter.UpdateCommand.Parameters["@OLD_" + column.ColumnName].Value =
                ...
                try {
                    ...
                    throw new DataException(
                        "Error on executing SQL command to save " + TableName + ":" + Environment.NewLine
                        + ex.Message + Environment.NewLine + Environment.NewLine
                        + "SQL command text:" + Environment.NewLine + Environment.NewLine
        ...
        protected virtual EntityColumnList CreateColumns() {
            ...
            foreach (PropertyInfo property in properties) {
                ...
                if (propertyIsField) {
                    ...
                    ReferencedFieldAttribute referencedFieldAttribute = (ReferencedFieldAttribute)(
                        from Attribute attribute in customAttributes
                        where attribute.GetType().IsSubclassOf(typeof(ReferencedFieldAttribute))
                    ...
                    if (referencedFieldAttribute != null) {
                        ...
                        if (referencedFieldAttribute.Name.Contains(".")) {
                            string[] chunks = referencedFieldAttribute.Name.Split('.');
                        ...
                        IEntity referencedEntity = Factory<IEntity>.Create(referencedTableName);
                        IEntityColumn referencedColumn =
                        ...
                        if (referencedColumn.DataType != property.PropertyType) {
                            throw new ApplicationException(
                                "Data type " + referencedColumn.DataType.ToString()
                                + " of field property "
                                + referencedColumnName
                                + " of referenced Entity class "
                                + referencedTableName
                                + " is not the same as data type " + property.PropertyType
                    ...
                    column.IsInPrimaryKey = ((
                        from Attribute attribute in customAttributes
                        where attribute.GetType().IsSubclassOf(typeof(PrimaryKeyFieldAttribute))
                        || attribute.GetType() == typeof(PrimaryKeyFieldAttribute)
                        select attribute).Count() >= 1);
                    column.IsInUniqueKey = ((
                        from Attribute attribute in customAttributes
                        where attribute.GetType().IsSubclassOf(typeof(UniqueKeyFieldAttribute))
        ...
        /// The target Entity.
        /// </param>
        private void DeeplyCopyEntityProperties(IEntity sourceEntity, IEntity targetEntity) {
            PropertyInfo[] properties = sourceEntity.GetType().GetProperties(
            ...
            foreach (PropertyInfo property in properties) {
                if (property.CanWrite) {
                    ...
                    || property.PropertyType == typeof(string)) {
                    ...
                    } else if (property.PropertyType.GetInterfaces().Contains(typeof(IEntityList))) {
                        ...
                        if (property.Name == "Credits") {
                            IEntityList sourceChildEntities = (IEntityList)property.GetValue(sourceEntity, null);
                            IEntityList targetChildEntities = Factory<IEntityList>.Create(
                                property.PropertyType,
                                // Constructor arguments
                                true); // empty
                            foreach (IEntity sourceChildEntity in sourceChildEntities) {
                                IEntity targetChildEntity = Factory<IEntity>.Create(sourceChildEntity.GetType());
                                // Recursively copy the properties of the source child Entity to the target child Entity.
                                DeeplyCopyEntityProperties(sourceChildEntity, targetChildEntity);
                        ...
                        if (property.PropertyType != sourceEntity.GetType()) {
                            ...
                            // See comment above.
                            IEntity sourceSubEntity = (IEntity)property.GetValue(sourceEntity, null);
                            IEntity targetSubEntity =
                        ...
                        /// Piece.AudioTags
                        PieceAudioTags audioTags = Factory<PieceAudioTags>.Create(
