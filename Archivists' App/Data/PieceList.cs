...
namespace SoundExplorers.Data {
    ...
    internal class PieceList : EntityList<Piece> {
        ...
        /// The Piece at the specified index in the list.
        /// </returns>
        public new Piece this[int index] {
            get {
                return base[index] as Piece;
            }
        }
        ...
        private static SelectCommand<Piece> CreateSelectCommand(IEntity owner) {
            ...
            if (owner != null) {
                ...
                foreach (IEntityColumn column in (new Piece()).Columns) {
                    ...
                    && column.ColumnName == owner.TableName) {
                        if (owner.PrimaryKeyColumns.ContainsKey(column.ReferencedColumnName)) {
                            ...
                            //   Act.Name,
                            //   Set.SetNo
                            referencedColumn = owner.PrimaryKeyColumns[column.ReferencedColumnName];
                        } else if (owner.UniqueKeyColumns.ContainsKey(column.ReferencedColumnName)) {
                            // True for Location.Name
                            referencedColumn = owner.UniqueKeyColumns[column.ReferencedColumnName];
                        }
                    }
                    if (referencedColumn != null) {
                        ...
                        parameter.DbType = SelectCommand<Piece>.TypeToDbType(column.DataType);
            ...
            if (selectCommand.Parameters.Count == 0) {
                ...
                foreach (IEntityColumn ownerPrimaryKeyColumn in owner.PrimaryKeyColumns) {
                    // Artist.Name
                    var parameter = new PgSqlParameter();
                    parameter.ParameterName = "@" + ownerPrimaryKeyColumn.ColumnName;
                    parameter.Value = ownerPrimaryKeyColumn.GetValue(owner);
                    parameter.DbType = SelectCommand<Piece>.TypeToDbType(ownerPrimaryKeyColumn.DataType);
                    selectCommand.Parameters.Add(parameter);
                }//End of foreach
            }
