...
namespace SoundExplorers.Data {
    ...
    internal abstract class EntityList<T> : List<IEntity>, IEntityList     
        ...
        public EntityColumnList NonPrimaryKeyColumns {
            get {
                return Entity.NonPrimaryKeyColumns;
            }
        }
        ...
        public EntityColumnList NonUniqueKeyColumns {
            get {
                return Entity.NonUniqueKeyColumns;
            }
        }
        ...
        public EntityColumnList UniqueKeyColumns {
            get {
                return Entity.UniqueKeyColumns;
            }
        }
        ...
        protected EntityList(
            ...
            Adapter.RowUpdated += new PgSqlRowUpdatedEventHandler(Adapter_RowUpdated);
            Adapter.RowUpdating += new PgSqlRowUpdatingEventHandler(Adapter_RowUpdating);
            ...
            if (parentListType == null) {
                ...
                try {
                    if (parentListType == typeof(PieceList)) {
                        ParentList = new PieceList(parentListType: null);
                    } else {
                ...
                DataColumn[] parentColumns = ParentKeyColumnsToDataColumns();
                DataColumn[] childColumns = ForeignKeyColumnsToDataColumns();
                ...
                try {
                    if (e.Errors.Message.ToLower().Contains("duplicate key")) {
                        exception = CreateDuplicateKeyException(
                            exception: e.Errors as PgSqlException,
                            rejectedValues: rejectedValues,
                            errorColumnIndex: ref errorColumnIndex);
        ...
        private void Adapter_RowUpdating(object sender, PgSqlRowUpdatingEventArgs e) {
            switch (e.StatementType) {
            case StatementType.Update:
            case StatementType.Delete:
                int rowIndex = Table.Rows.IndexOf(e.Row);
        ...
        private ApplicationException CreateDuplicateKeyException(
            ...
            foreach (IEntityColumn keyColumn in keyColumns) {
                string errorValue =
                    keyColumn.DataType == typeof(DateTime)
                    ? ((DateTime)rejectedValues[Columns.IndexOf(keyColumn)]).ToString("dd MMM yyyy")
        ...
        /// or:
        ///     update or delete on table "TABLE" violates foreign key constraint "FOREIGNKEY" on table "REFERENCINGTABLE"
        /// </remarks>
        /// <exception cref="NotSupportedException">
        /// Thrown if a meaningful error message cannot be generated by parsing
        /// the error message generated by the database engine.
        ...
        private ApplicationException CreateForeignKeyException(
                ...
                object[] rejectedValues,
                ref int errorColumnIndex) {
            string[] chunks = exception.Message.Split('"');
            ...
            string notSupportedMessage =
                ...
                + "\" is not supported." + Environment.NewLine
                + "It should consist of either" + Environment.NewLine 
                + "\"fk_" + TableName.ToLower()
                + "_\" followed by the referenced table name" + Environment.NewLine
                + "or" + Environment.NewLine 
                + "\"fk\" followed by the referencing table name " 
                + "followed by \"_\" followed by \"" + TableName.ToLower() + "\"."
            ...
            string referencedEntityName = (
                ...
                && column.ReferencedTableName.ToLower() == referencedTableName.ToLower()
            ...
            IEntity referencedEntity = Factory<IEntity>.Create(referencedEntityName);
            var foreignKeyColumns = new EntityColumnList();
            foreach (IEntityColumn column in Columns) {
            ...
            foreach (IEntityColumn foreignKeyColumn in foreignKeyColumns) {
                string errorValue =
                    foreignKeyColumn.DataType == typeof(DateTime)
                    ? ((DateTime)rejectedValues[Columns.IndexOf(foreignKeyColumn)]).ToString("dd MMM yyyy")
        ...
        private DataColumn[] ForeignKeyColumnsToDataColumns() {
            var dataColumns = new List<DataColumn>(ParentList.PrimaryKeyColumns.Count());
            foreach (IEntityColumn parentKeyColumn in ParentList.PrimaryKeyColumns) {
                foreach (IEntityColumn mainKeyColumn in this.PrimaryKeyColumns) {
                    if (mainKeyColumn.ColumnName == parentKeyColumn.ColumnName
                    ||  mainKeyColumn.ReferencedColumnName == parentKeyColumn.ColumnName) {
                        dataColumns.Add(this.Table.Columns[mainKeyColumn.ColumnName]);
                        break;
                    }
                }//End of foreach
            }//End of foreach
        ...
        private void PopulateEntityFromRow(IEntity entity, DataRow row) {
            foreach (IEntityColumn column in Entity.Columns) {
                if (!column.IsHidden) {
                    ...
                    try {
                        ...
                        throw new ApplicationException(
                            ...
                            + " data type " + column.DataType.ToString()
